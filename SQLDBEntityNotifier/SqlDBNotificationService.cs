...
namespace SQLDBEntityNotifier
{
    ...
    public class SqlDBNotificationService<T> : IDBNotificationService<T> where T : class, new()
    {
        ...
        private readonly Func<long, string> _changeTrackingQueryFunc;
        ...
        : this(changeTableService, tableName, connectionString, version, period, null) { }
            ...
            Func<long, string>? changeTrackingQueryFunc)
        {
            ...
            _changeTrackingQueryFunc = changeTrackingQueryFunc ?? (ver => $"SELECT ct.* FROM CHANGETABLE(CHANGES {_tableName},{ver}) ct WHERE ct.SYS_CHANGE_VERSION <= {{0}}");
        }
        ...
        public async Task PollForChangesAsync()
        {
            ...
            try
            {
                ...
                string commandText = _changeTrackingQueryFunc(_currentVersion).Replace("{0}", lastVersion.ToString());
